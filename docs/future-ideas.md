...
**Privacy caveat in v1**: because there is no authentication and the `calculations` table on Neon has no `user_id` foreign key, every visitor reads from and writes to the same shared history. Anyone hitting the deployed site can see the inputs and outputs that previous visitors priced. This is an intentional v1 simplification (it lets the project ship faster and keeps the data model small), not a bug. It is also the most concrete reason to graduate to per user history if the deployed link is ever shared somewhere visitors might price something they would prefer to keep private.
...
**When to revisit**: after Phase 11 is shipped and stable, or earlier if the project grows beyond a single user demo, or if the shared history starts to feel like an exposure (a recruiter, an interviewer, or any visitor would see whatever the last person typed).
